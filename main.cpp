...
void Add_To_Product_Line(Product_Info &my_Product_Info, std::vector<Product_Info> &new_Product_Info) {

    // Add three new products to the product line
    Assemble_New_Product_Info(my_Product_Info, new_Product_Info);
    Assemble_New_Product_Info(my_Product_Info, new_Product_Info);
    Assemble_New_Product_Info(my_Product_Info, new_Product_Info);
    // Output the products in the product line
    Display_New_Added_Products(my_Product_Info, new_Product_Info);
    Store_New_Products_To_File(my_Product_Info, new_Product_Info);

}

void Assemble_New_Product_Info(Product_Info &my_Product_Info, std::vector<Product_Info> &new_Product_Info) {
    ...
    my_Product_Info.product_Line_Manufacturer = entered_Manufacturer;
    std::cout << "Enter the Product Name\n";
    std::string entered_product_Name;
    getline(std::cin, entered_product_Name);
    my_Product_Info.product_Line_Name = entered_product_Name;
    ...
    my_Product_Info.product_Line_Item_Type = entered_item_Type_Code;
    new_Product_Info.push_back(my_Product_Info);

}


void Display_New_Added_Products(const Product_Info &my_Product_Info, std::vector<Product_Info> &new_Product_Info) {
...
void Store_New_Products_To_File(const Product_Info &my_Product_Info, std::vector<Product_Info> &new_Product_Info) {
    for (auto &product_Line_Item_Num : new_Product_Info) {
...
void Loads_Products_From_ProductLine_File(Product_Info &my_Product_Info, std::vector<Product_Info> &catalog) {
    ...
    while (getline(catalog_file, line_In_File)) {
        ...
        if (line_In_File.empty()) { //skips lines that are empty in the ProductLine file
            ...
            my_Product_Info.product_Line_Manufacturer = manufacturer_In_File;
            my_Product_Info.product_Line_Name = product_Name_In_File;
            my_Product_Info.product_Line_Item_Type = item_Type_In_File;
            catalog.push_back(my_Product_Info);
        }
    }
